...
describe("JobList for non-authenticated users", () => {
  it("renders", async () => {
  ...
  it("submits search with title filter", async () => {
    const { useJobs } = await import("../hooks/useJobs");
    const user = userEvent.setup();
    render(<JobList />, { wrapper });

    await waitFor(() => screen.getByPlaceholderText("Job Title"));

    await user.type(screen.getByPlaceholderText("Job Title"), "Backend");
...
describe("JobList for authenticated users", () => {
  it("shows detail page link, and edit and delete buttons for an authenticated user", async () => {
